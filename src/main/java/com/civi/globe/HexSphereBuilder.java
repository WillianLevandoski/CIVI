...
final class HexSphereBuilder {
    ...
    void build(int n, double r) {
        ...
        for (int a = 0; a < 5; a++) {
            ...
            h0.ix[a] = points.add(px, py, +pz);
            h1.ix[a] = points.add(px, py, -pz);
        }
        h0.ix[5] = h0.ix[4];
        h1.ix[5] = h1.ix[4];
        cells.add(h0);
        ...
        for (int a = 0; a < na; a += n - 2) {
            double ang = (36.0 * DEG) - ((a / (double) (n - 2)) * 72.0 * DEG);
